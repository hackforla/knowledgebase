...
export const isCodeBlocks = (table: any) => {
  ...
  const hasMonospaceFont = firstCell.content.every(({ paragraph }: any) =>
    paragraph.elements.every(({ textRun }: any) => {
...
export const isQuote = (table: any) => {
  const hasOneCell = table.rows === 1 && table.columns === 1;
